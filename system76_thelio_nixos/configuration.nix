...
in
{
  ...
  services.unix-mail-redux = {
    ...
    # A live 2026-09-03 proof showed the temporary PTY client could disconnect
    # Codex without delivering the wake. Keep Codex passive until a native
    # app-server wake exists; Claude and Grok retain their terminal wake path.
  ...
  users.users.pmarreck = {
    ...
    packages = with pkgs; [
      # The official Codex GUI shares Peter's ~/.codex state and belongs in his
      # profile, not every user's system-wide PATH. It is defined and tested as
      # /etc/nixos#codex-app, then injected here through flake specialArgs.
      codexApp
      # Terminal Code packages its pinned Electron + code-server payload through
      # /etc/nixos#tode; no curl-to-shell installer or mutable self-upgrade.
      tode
